...
export class AddExeedTripApprovalUpdateTripUpdateNotification1774338262315 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "public"."exceed_approval_status_enum" AS ENUM('pending', 'approved', 'rejected', 'canceled')`);
        await queryRunner.query(`CREATE TABLE "exceed_approval" ("id" SERIAL NOT NULL, "approverApprovedAt" TIMESTAMP, "approverComments" character varying(255), "Status" "public"."exceed_approval_status_enum" NOT NULL DEFAULT 'pending', "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "tripId" integer, "approver_id" integer, CONSTRAINT "REL_cbc3da6e22a930ec38daab057c" UNIQUE ("tripId"), CONSTRAINT "PK_629731be717884762dc5b4fab3b" PRIMARY KEY ("id"))`);
        await queryRunner.query(`ALTER TYPE "public"."trip_status_enum" RENAME TO "trip_status_enum_old"`);
        await queryRunner.query(`CREATE TYPE "public"."trip_status_enum" AS ENUM('draft', 'pending', 'canceled', 'approved', 'rejected', 'read', 'ongoing', 'finished', 'exceed', 'completed')`);
        ...
        await queryRunner.query(`ALTER TABLE "exceed_approval" ADD CONSTRAINT "FK_cbc3da6e22a930ec38daab057c7" FOREIGN KEY ("tripId") REFERENCES "trip"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "exceed_approval" ADD CONSTRAINT "FK_5ee8478ff1433b8c2f13da7a932" FOREIGN KEY ("approver_id") REFERENCES "user"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
